...
FM_SNAPSHOT_TASK_CACHE_DIR=$FM_HOME/state/fleet-task-cache   # private parent-side observation cache binding each task's current-state observation to the inputs it was computed from; reuse on unchanged inputs keeps the home-summary producer inside its 60-second publication deadline
FM_SNAPSHOT_RUNS_LIMIT=200         # maximum no-mistakes runs-table rows folded into the per-branch cache fingerprint; truncation keeps the fingerprint cheap, and any truncation still distinguishes branch rows from empty
FM_SNAPSHOT_RUNS_TIMEOUT=5         # seconds bounding the shared per-cycle no-mistakes runs-inventory call; on expiry the cycle reuses the last-good inventory (kept beside the task observations) so unchanged tasks keep their cached observations, and only a cycle that never captured one falls back to live per-task reads
